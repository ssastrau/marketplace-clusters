...
export MARKETPLACE_APP="apps/linode-marketplace-glusterfs"
...
function rename_provisioner {
	...
	curl -s -H "Content-Type: application/json" \
		-H "Authorization: Bearer ${TOKEN_PASSWORD}" \
		-X PUT -d "{
        \"label\": \"glusterfs-instance-1-${UUID}\"
      }" \
...
EOF
  ...
  if [[ -n ${DOMAIN} ]]; then
	...
	if [[ -n ${CLUSTER_SIZE} ]]; then
		echo "cluster_size: ${CLUSTER_SIZE}" >>${group_vars}
	fi

	if [[ -n ${CLIENT_IPS} ]]; then
		IPS=$(echo "${CLIENT_IPS}" | tr ',' '\n' | grep -Eo '^[[:space:]]*([0-9]{1,3}\.){3}[0-9]{1,3}(/[0-9]+)?' | sed 's/^[[:space:]]*//' | sed 's/^/  - /g')
		if [ -n "${IPS}" ]; then
			echo "[info] Adding client IPs to group_vars..."
			printf 'client_ips:\n%s\n' "${IPS}" >>${group_vars}
		else
			echo "[info] No valid client IP addresses found, skipping"
		fi
	fi
