...
if [[ -n "${CLUSTER_NAME}" ]]; then
        ...
        UDF_VARS["CLUSTER_NAME"]="Cassandra Cluster" # default
fi

if [[ -n "${DB_SUPERUSER}" ]]; then
        UDF_VARS["DB_SUPERUSER"]="${DB_SUPERUSER}"
else
        UDF_VARS["DB_SUPERUSER"]="db_admin" # default
fi

if [[ -n "${CASSANDRA_VERSION}" ]]; then
        UDF_VARS["CASSANDRA_VERSION"]="${CASSANDRA_VERSION}"
else
        UDF_VARS["CASSANDRA_VERSION"]="4.1.5" # default
fi
...
if [[ -n "${CLIENT_COUNT}" ]]; then
        ...
        UDF_VARS["CLIENT_COUNT"]="3" # default
fi
