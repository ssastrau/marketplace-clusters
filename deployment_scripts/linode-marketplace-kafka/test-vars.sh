...
if [[ -n "${KAFKA_VERSION}" ]]; then
        UDF_VARS["KAFKA_VERSION"]="${KAFKA_VERSION}"
else
        UDF_VARS["KAFKA_VERSION"]="3.8.0" # default
fi
...
if [[ -n "${EMAIL_ADDRESS}" ]]; then
        ...
        UDF_VARS["EMAIL_ADDRESS"]="webmaster@example.com" # default
fi

if [[ -n "${CA_COMMON_NAME}" ]]; then
        UDF_VARS["CA_COMMON_NAME"]="${CA_COMMON_NAME}"
else
        UDF_VARS["CA_COMMON_NAME"]="Kafka RootCA" # default
fi
