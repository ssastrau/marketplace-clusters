...
def main():
    ...
    for collection_name in all_collections:
        if '.' in collection_name:
            ...
            if version:
                latest_versions[collection_name] = version
    ...
    for f in collections_files:
        ...
        for col_name, col_version in pinned.items():
            new = latest_versions.get(col_name)
            if not new or new == col_version:
                continue
            pattern = rf"(name:\s+{re.escape(col_name)}\s*\n\s*version:\s+[\"']?){re.escape(col_version)}([\"']?)"
            content, count = re.subn(pattern, rf"\g<1>{new}\g<2>", content)
            if count > 0:
                report.append(f'| `{app}` | `{col_name}` | `{col_version}` | `{new}` |')
                changed = True
        if changed:
