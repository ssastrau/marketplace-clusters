...
def remote_exec(ssh_credentials):
    ...
    Services bound to loopback or a private network are reached from the box itself
    rather than from the test runner.

    Usage in a test/service:
        out, err, code = remote_exec("systemctl is-active kibana")
...
def http_session():
    ...
    test deploys use self-signed *.ip.linodeusercontent.com certs.
